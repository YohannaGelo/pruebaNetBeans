...
import clasesAuxiliares.Employee;
...
public class EmployeeManagement {
    ...
    //variables que se declran fuera del main, deben ser estáticos
    ...
    private static boolean loadData (boolean load) {
        ...
        if (!load || reload)  {
            ...
            employeesArray = new Employee[s.nextInt()]; //par no crear una variable de tamaño
    ...
    private static void showData (Employee[] employeesArray, boolean load) {
    ...
    private static void showPayroll (Employee[] employeesArray, boolean load) {
        ...
        if (load) {
            for (int i = 0; i < employeesArray.length; i++) {
                ...
                double irpfRet = (employeesArray[i].irpfRetention()/100) *  employeesArray[i].baseSalary();
                double finalSalary = employeesArray[i].baseSalary() - irpfRet;


                System.out.print("Nombre empleado\t: " + employeesArray[i].getName() + "\nApellidos\t: " 
                        + employeesArray[i].getSurname() + "\nNº de hijos\t: " + employeesArray[i].getNumberChildren()
                        + "\nCategoría\t: " + employeesArray[i].profesionalCategory());
                
                //imprimo los decimales con printf para poder establecer solo 2 decimales.
                System.out.printf("\nSueldo base\t: %.2f Euros\nRetención\t: %.2f%%\n\nSUELDO TOTAL\t: %.2f Euros\n", employeesArray[i].baseSalary(), employeesArray[i].irpfRetention(), finalSalary);
    ...
    public static void main(String[] args) {
        ...
        while (repeat) {
            //lanzamos el menú
            int opt = EmployeeManagement.menu();
